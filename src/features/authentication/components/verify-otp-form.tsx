    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mb-7">
        <InputOTP onChange={handleOtpInputChange} value={otp} maxLength={5}>
          <InputOTPGroup className="w-full justify-between mb-2">
            {[...Array(5)].map((_, index) => (
              <InputOTPSlot
                className="h-[60px] w-[60px] border-[1px] rounded-lg text-[26px] text-dark-gray-250"
                key={index}
                index={index}
              />
            ))}
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <CustomButton
          ...
          fontSize="text-sm"
          fontWeight="medium"
          color="text-blue-tone-300"
          width="w-[143px]"
          classNames="gap-1"
          leftIcon={<RefreshCcw className="h-6 w-4 text-dark-gray-300" />}
        />
      </div>
      ...
      <CustomButton
        ...
        classNames="gap-1"
        rightIcon={
          <Text variant="pm" color="text-gold" fontWeight="semibold">
            Sign In
          </Text>
        }
      />
