...
const usePublicSearch = (isSearchPage = false) => {
  ...
  const handleLoadStatusChange = useCallback(
    (value: { label: string; value: string } | null) => {
  ...
  const areRequiredFieldsPresent = useMemo(() => {
    return !!(selectedProduct && depot && selectedSize && selectedLoadStatus);
  }, [selectedProduct, depot, selectedSize, selectedLoadStatus]);
  ...
  const loadStatus = LOAD_STATUS_OPTIONS;
  ...
  useEffect(() => {
    if (isSearchPage && searchParams) {
      ...
      if (productId || depotHubId || sizeValue || loadStatusValue) {
        ...
        if (loadStatusValue && loadStatus?.length) {
          const loadStatusObj = loadStatus.find(
            (item: any) => item.value === loadStatusValue,
          );
          console.log(loadStatusObj, loadStatus, 'Fff');
          if (loadStatusObj) setSelectedLoadStatus(loadStatusObj);
        }
  ...
  }, [isSearchPage, searchParams, products, depots, loadStatus]);
  ...
  const constructedSearchQuery = useMemo(() => {
    if (
      ...
      return `?productId=${selectedProduct.value}&depotHubId=${depot.value}&size=${selectedSize.value}&loadStatus=${selectedLoadStatus.value}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               &status=available&limit=20&page=`;
    }
  ...
  useEffect(() => {
    if (
      ...
      const query = `?productId=${selectedProduct.value}&depotHubId=${depot.value}&size=${selectedSize.value}&loadStatus=${selectedLoadStatus.value}&status=available&limit=20&page=`;
  ...
  const handleSearchTruckClick = useCallback(() => {
    ...
    if (!selectedProduct || !depot || !selectedSize || !selectedLoadStatus) {
    ...
    if (isSearchPage) {
      ...
      window.history.pushState(
        {},
        '',
        `/truck-search?productId=${selectedProduct.value}&depotHubId=${depot.value}&size=${selectedSize.value}&loadStatus=${selectedLoadStatus.value}`,
      );
